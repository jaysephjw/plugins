...
- (instancetype)initWithAsset:(NSString*)asset frameUpdater:(FLTFrameUpdater*)frameUpdater {
  NSString* path = [[NSBundle mainBundle] pathForResource:asset ofType:nil];
    return [self initWithURL:[NSURL fileURLWithPath:path] frameUpdater:frameUpdater cookies: nil];
}
...
- (instancetype)initWithURL:(NSURL*)url frameUpdater:(FLTFrameUpdater*)frameUpdater cookies:(NSArray*)cookies {
  AVPlayerItem *item;
  if (cookies == (id)[NSNull null] || cookies == nil) {
    item = [AVPlayerItem playerItemWithURL:url];
  } else {
    NSMutableArray* httpCookies = [NSMutableArray new];
    for (NSString* cookieString in cookies) {
      NSArray* parsedCookies = [NSHTTPCookie cookiesWithResponseHeaderFields: @{@"Set-Cookie": cookieString}
                                                                      forURL: [NSURL URLWithString: @""]];
      [httpCookies addObjectsFromArray: parsedCookies];
    }
    AVURLAsset* asset = [AVURLAsset URLAssetWithURL:url options:@{AVURLAssetHTTPCookiesKey: httpCookies}];
